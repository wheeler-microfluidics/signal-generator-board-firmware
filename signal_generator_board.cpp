  ...
  if(Serial.available()) {
    ...
    if(substr && substr==buffer_ && substr[strlen(substr)-1] == ')') {
      ...
      if(NextInt(substr, index)) {
        if(index>=0 && index<6) {
          Serial.println(P("pot[") + String(index) + "]=" + String(pot_[index], DEC));
          return;
        }
      }
    }
    ...
    if(substr && substr==buffer_ && substr[strlen(substr)-1] == ')') {
      ...
      NextInt(substr, save_to_eeprom);
      if(index>=0 && index<6 && value>=0 & value < 256) {
        set_pot((uint8_t)index, (uint8_t)value, (boolean)save_to_eeprom);
        return;
      } else {
        Error(1);
        return;
    ...
    if(substr && substr==buffer_ && substr[strlen(substr)-1] == ')') {
      buffer_[strlen(substr)-1]=0;
      set_waveform_voltage(atof(substr+sizeof("set_waveform_voltage(")-1));
      return;
    }
...
void SignalGeneratorClass::ProcessWireCommand() {
  ...
  case CMD_SET_WAVEFORM_VOLTAGE:
    if(payload_length_==sizeof(float)) {
      return_code_ = set_waveform_voltage(ReadFloat());
    } else {
      return_code_ = RETURN_BAD_PACKET_SIZE;
    }
    break;
  case CMD_GET_POT_VALUE:
    if(payload_length_==sizeof(uint8_t)) {
      ...
      Serialize(&pot_[index], sizeof(pot_[index]));
      return_code_ = RETURN_OK;
    } else {
      return_code_ = RETURN_BAD_PACKET_SIZE;
    }
    break;
  case CMD_SET_POT_VALUE:
    if(payload_length_==2*sizeof(uint8_t)) {
      uint8_t index = ReadUint8();
...
void SignalGeneratorClass::LoadConfig(bool use_defaults) {
  ...
  for(uint8_t i=0; i<6; i++) {
    set_pot(i, config_settings_.pot[i], false, false);
  }
}
...
void SignalGeneratorClass::set_pot(byte index, byte value, boolean save_to_eeprom, boolean display_msg) {
  ...
  if(display_msg) {
    if(save_to_eeprom) {
      Serial.println(P("pot[") + String(index) + "]=" + String(value, DEC) + P(" (save to EEPROM)"));
    } else {
      Serial.println(P("pot[") + String(index) + "]=" + String(value, DEC));
    }
  }
}
